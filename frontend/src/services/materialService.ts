...
export async function getMaterials(): Promise<Material[]> {
    ...
    if (!response.ok) {
        throw new Error("Nije moguće učitati materijale.");
    }
...
export async function createMaterial(
    ...
    formData.append("pages", material.pages.toString());
    ...
    if (!response.ok) {
        ...
        throw new Error(
            message || "Nije moguće dodati materijal."
        );
    }
...
export async function getMyMaterials(): Promise<Material[]> {
    ...
    if (response.status === 401) {
        throw new Error(
            "Moraš biti prijavljen da bi vidio svoje materijale."
        );
    }
